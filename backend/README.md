...
- Sample:  `curl http://127.0.0.1:5000/quizzes -X POST -H "Content-Type: application/json" -d '{"previous_questions": [16,19], "quiz_category": {"type":"Art", "id":2}}'`
...
- General:
  - Send a POST request to add a new question
    ...
    "answer": "Here is a new response string",
    "difficulty": 1,
    "category": "3",
    }
     ```
  - Returns: does not return new data
- Sample: `curl http://127.0.0.1:5000/question -X POST -H "Content-Type: application/json" -d{"question":"Some question","answer":"T","difficulty":2,"category":6}'`
...
- General:
  - Send a POST request to search for a specific question by search term
    Request body:
     ```json
    {
      "searchTerm": "here is the search term"
    }
     ```
  - Returns: any array of questions, a number of questions that match the search term and the current          category  string
- Sample: `curl -X POST http://127.0.0.1:5000/questions -H "Content-Type:application/json" -d'{"searchTerm":"Egyp"}'`
